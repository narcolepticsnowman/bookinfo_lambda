...
public class LambdaHandler implements RequestHandler<APIGatewayV2HTTPEvent, APIGatewayV2HTTPResponse> {
    ...
    private JsonObject getRatings(String productId, Map<String, String> requestHeaders) {
        ...
        for (String header : headers_to_propagate) {
            requestHeaders.values().forEach(v -> builder.header(header, v));
    ...
    public APIGatewayV2HTTPResponse handleRequest(APIGatewayV2HTTPEvent event, Context context) {
        ...
        if (ratings_enabled) {
            JsonObject ratingsResponse = getRatings(productId, event.getHeaders());
        ...
        return APIGatewayV2HTTPResponse.builder()
                .withStatusCode(200)
                .withHeaders(Map.of("content-type", MediaType.APPLICATION_JSON))
                .withBody(jsonResStr)
                .build();
